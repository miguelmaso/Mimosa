...
function StateEquationIter(x0,Λ, loadinc::Int64, ndofm, cache)
    #----------------------------------------------
    ...
    println("+++ Loadinc is  with a load factor of    +++\n")
    cacheold = cache
...
function StateEquation(φmax::Float64; fem_params)
    ...
    while (φap / φmax) < 1.0 - 1e-6
        φap += φ_inc
        φap = min(φap, φmax)
        x0, cache, flag = StateEquationIter(x0,ϕap/ϕmax, loadinc, fem_params.ndofm, cache)
        if (flag == false)
            φap -= φ_inc
            φ_inc = φ_inc / 2
            nbisect += 1
