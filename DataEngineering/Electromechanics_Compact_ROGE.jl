...
mesh_file = joinpath(dirname(@__FILE__), "parametrize_plate_elec_ROGE.msh")
...
#    writevtk(model, model_file)
    ...
    fem_params = (; Ωₕ, dΩ, ndofm, ndofe, Uφᵛ,Uuv,  Uu,    Uφˢt1,Uuˢt1, Uφˢt2, Uuˢt2, Uφˢb1, Uφˢb2, Qₕ)
    ...
    function StateEquation(target_gen,ϕ_app::Vector; fem_params)
        ...
        while Λ < 1.0 - 1e-6
            ...
            if (flag == true)
                ...
                u_Vector_on_Surface   =  vcat(u_Vector_on_Surface1,u_Vector_on_Surface2)
                ...
                X_Vector_on_Surface  =  vcat(X_Vector_on_Surface1,X_Vector_on_Surface2)                
#                cd("Potential_$folder_name")
                filename = join(string.(round.(Λ*ϕ_app,digits=4)), "_")


                # open("$filename.txt","w") do io
                #     writedlm(io,u_Vector_on_Surface)
                # end
                # open("simple_mat_coords.txt","w") do io
...
input_file = readdlm("LHS.txt")
input_indices = rand(1:size(input_file,2),10)
...
# end
#result_folder = "./results/Paraview_results_Compact_Roge/"
