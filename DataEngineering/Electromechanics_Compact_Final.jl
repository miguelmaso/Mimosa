...
result_folder = "./results/FE_Results_V3/"
...
#    writevtk(model, model_file)
    ...
    fem_params = (; Ωₕ, dΩ, ndofm, ndofe, Uφᵛ,Uuv, Uφˢt1,Uuˢt1,Uuˢt2, Uφˢt2, Uφˢb1, Uφˢb2, Qₕ)
    ...
    function StateEquation(target_gen,ϕ_app::Vector; fem_params)
        ...
        while Λ < 1.0 - 1e-6
            ...
            if (flag == true)
                ...
                u_Projected_1 = interpolate_everywhere(u_Fe_Function, fem_params.Uuˢt1) #Interpola en una superficie la FE
                u_Projected_2 = interpolate_everywhere(u_Fe_Function, fem_params.Uuˢt2) #Interpola en una superficie la FE
                u_Vector_on_Surface_1 = get_free_dof_values(u_Projected_1) # Saca un vector
                u_Vector_on_Surface_2 = get_free_dof_values(u_Projected_2) # Saca un vector
                u_Vector_on_Surface=hcat(u_Vector_on_Surface_1,u_Vector_on_Surface_2)
                f_mat(x) = x
                X_projected_1 = interpolate_everywhere(f_mat, fem_params.Uuˢt1)
                X_projected_2 = interpolate_everywhere(f_mat, fem_params.Uuˢt2)
                X_Vector_on_Surface_1 = get_free_dof_values(X_projected_1)
                X_Vector_on_Surface_2 = get_free_dof_values(X_projected_2)
                X_Vector_on_Surface = hcat(X_Vector_on_Surface_1,X_Vector_on_Surface_2)
...
mkdir("New_FE_Data_Trajectories")
cd("New_FE_Data_Trajectories")
